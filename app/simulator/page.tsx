...
function deriveRate(
  ...
  usage: UsageType
): { base: number; adjusted: number; adjustments: RateAdjustment[] } {
  ...
  if (vesselAge != null) {
    if (vesselAge > 25) {
      ...
      adjustments.push({ label: "New / near-new vessel", deltaPct: -0.2 });
    }
  }

  if (usage === "commercial_charter") {
    adjustments.push({ label: "Commercial charter usage", deltaPct: 0.3 });
...
function Label({ children, right }: { children: React.ReactNode; right?: React.ReactNode }) {
  return (
    ...
    >
      ...
      {right && <div style={{ fontSize: 13, color: C.gray2, fontWeight: 700 }}>{right}</div>}
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <input
        ...
        style={{
          width: "100%",
          padding: prefix ? "12px 14px 12px 6px" : "12px 14px",
          border: "none",
...
export default function SimulatorPage() {
  ...
  const [manualRate, setManualRate] = useState<number>(5.8);
  ...
  const monthlyPayment = useMemo(() => calcMonthlyPayment(loan, effectiveRate, termYears), [loan, effectiveRate, termYears]);
        ...
        <section style={{ padding: "12px clamp(20px,5vw,80px) 0" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            ...
            <p style={{ color: C.gray2, fontSize: 15, lineHeight: 1.7, maxWidth: 640, marginBottom: 32 }}>
              Adjust vessel details to get a derived indicative rate — then explore monthly payments or calculate your borrowing
              capacity.
            </p>
          </div>
        </section>
              ...
              >
                ...
                >
                  <div style={{ marginBottom: 18 }}>
                    ...
                    <InputField value={fmtInput(price)} onChange={(v) => setPrice(parseMoney(v))} prefix={sym(currency)} inputMode="numeric" />
                  </div>

                  <div style={{ marginBottom: 18 }}>
                    <Label right={`${fmtMoney(deposit, currency)} (${price > 0 ? Math.round((deposit / price) * 100) : 0}%)`}>Deposit</Label>
                    <Slider
                      min={0}
                    ...
                    <div className="input-row-2" style={{ display: "grid", gridTemplateColumns: "1fr 160px", gap: 10, marginTop: 10 }}>
                      <InputField value={fmtInput(deposit)} onChange={(v) => setDeposit(parseMoney(v))} prefix={sym(currency)} inputMode="numeric" />
                      <div
                        style={{
                          padding: "10px 14px",
                          borderRadius: 12,
                  ...
                  {!useSmartRate && (
                    <div style={{ marginBottom: 18 }}>
                      ...
                      <Slider min={3.5} max={11.5} step={0.05} value={manualRate} onChange={setManualRate} />
                    </div>
                  )}
                ...
                >
                  <div style={{ marginBottom: 18 }}>
                    ...
                    <InputField
                      ...
                      inputMode="numeric"
                    />
                    <Slider min={2000} max={100000} step={500} value={monthlyBudget} onChange={setMonthlyBudget} />
                  </div>

                  <div style={{ marginBottom: 18 }}>
                    <Label>Available deposit</Label>
                    <InputField value={fmtInput(capacityDeposit)} onChange={(v) => setCapacityDeposit(parseMoney(v))} prefix={sym(currency)} inputMode="numeric" />
                  </div>

                  <div style={{ marginBottom: 18 }}>
                    <Label>Loan term</Label>
                    <SelectField
                      value={String(capacityTerm)}
                      onChange={(v) => setCapacityTerm(Number(v))}
                      options={[
                        { value: "5", label: "5 years" },
                        { value: "8", label: "8 years" },
                        { value: "10", label: "10 years" },
