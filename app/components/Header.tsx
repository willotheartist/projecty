...
const Caret = ({ up }: { up?: boolean }) => (
  <span style={{ display: "inline-block", transform: up ? "rotate(180deg)" : "none" }}>▾</span>
);

const Arrow = () => <span style={{ opacity: 0.65, fontSize: 18 }}>›</span>;
...
function flattenMenu(m: MenuDef) {
  const groups = m.columns.map((c) => ({ title: c.title, items: c.items }));
  return groups;
}
  ...
  return (
    <>
      <style>{`
        ...
        .pill-cta:hover { box-shadow: 0 6px 18px rgba(37,99,235,0.25); transform: translateY(-1px); }
        ...
        /* DESKTOP DROPDOWN SITE LINKS (THIS is what you asked to enlarge + space out) */
        .mi-row {
          display: flex;
      ...
      {/* Desktop dropdown backdrop (ONLY for desktop mega menu) */}
      <div
        onClick={closeMenu}
        style={{
          position: "fixed",
        ...
        <>
          ...
          <div className="drawer" role="dialog" aria-modal="true" aria-label="Mobile navigation">
            <div className="drawer-head">
              <div className="drawer-logo">YFI</div>
            ...
            <div className="drawer-body">
              ...
              <div className="m-section">
                <button type="button" className="m-section-btn" onClick={() => alert("Case Studies")} aria-expanded={false}>
      ...
      >
        ...
        >
          <span style={{ fontFamily: "var(--font-serif), serif", fontSize: 30, cursor: "pointer", lineHeight: 1 }}>
            YFI
          </span>
          ...
          <div className="mobileOnly" style={{ alignItems: "center", gap: 10 }}>
            <button className="pill-cta" type="button" onClick={() => setMobileOpen(true)}>
              Menu
            </button>
          </div>
        </div>
